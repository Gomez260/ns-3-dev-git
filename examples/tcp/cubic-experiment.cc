...
main(int argc, char* argv[])
{
    ...
    bool tracing = false;
    std::string prefix_file_name = "CubicExperiment";
    ...
    bottleneckDevices = UnReLink.Install(gateways.Get(0), gateways.Get(1));

    if (queue_disc_type == "ns3::PfifoFastQueueDisc")
    {
        tchPfifo.Install(bottleneckDevices);
    }
    else if (queue_disc_type == "ns3::CoDelQueueDisc")
    {
        tchCoDel.Install(bottleneckDevices);
    }
    else
    {
    ...
    if (tracing)
    {
        ...
        for (uint16_t index = 0; index < num_flows; index++)
        {
            ...
            // Source IDs start at 2 because Gateway A and Gateway B are nodes 0 and 1
            uint32_t sourceNodeId = index + 2;

            // Sink IDs start after the gateways and all sources
            uint32_t sinkNodeId = num_flows + index + 2;

            firstCwnd[sourceNodeId] = true;
            firstSshThr[sourceNodeId] = true;
            firstRtt[sourceNodeId] = true;
            firstRto[sourceNodeId] = true;
